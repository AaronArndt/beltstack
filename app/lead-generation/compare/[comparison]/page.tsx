...
export async function generateMetadata({ params }: Props) {
  const { comparison } = await params;
  const data = getLeadGenerationComparisonBySlug(comparison);
  if (!data) return { title: "Compare Lead Generation Tools" };
  const year = new Date().getFullYear();
  return {
    title: `${data.productA.name} vs ${data.productB.name} (${year}) | BeltStack Comparison`,
    description: `Compare ${data.productA.name} vs ${data.productB.name} for local service businesses. See pricing signals, lead models, and which channel fits your trade.`,
  };
}
