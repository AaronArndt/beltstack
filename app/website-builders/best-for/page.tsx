      ...
      <main>
        <section className="bg-background">
          <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 sm:py-10 lg:px-8 lg:py-12">
            ...
            <p className="mt-3 text-[#57534E] text-base leading-relaxed max-w-3xl">
              Find recommendations by service-business audience, trade, and growth stage.
            </p>
          </div>
        </section>
        ...
        <section className="scroll-mt-section border-b border-stone-200/80 bg-white py-8 sm:py-11">
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <SectionTitle sub="Choose the page that matches your business profile.">
              Website builder best-for pages
            </SectionTitle>
            <div className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {WEBSITE_BUILDERS_BEST_FOR_INDEX.map((item) => (
                <Link key={item.href} href={item.href} className="flex flex-col rounded-lg border border-stone-200 bg-white p-5 shadow-sm hover:border-stone-300">
                  <h3 className="text-[#1A2D48] font-bold">{item.label}</h3>
                  <p className="mt-1 text-[#57534E] text-sm leading-relaxed">{item.description}</p>
                  <span className="mt-3 inline-block text-sm font-semibold text-[#10B981]">See picks →</span>
                </Link>
              ))}
            </div>
          </div>
