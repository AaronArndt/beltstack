...
const sectionSubClass = "mt-1 text-[#6E6E6E] text-sm sm:text-base";
      ...
      <main>
        <section className="border-b border-slate-200 bg-[#F8FAFC]">
          <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6 sm:py-12 lg:px-8 lg:py-14">
            ...
            <p className="mt-3 max-w-3xl text-[#6E6E6E] text-base leading-relaxed">
        ...
        <section id="popular-categories" className="border-b border-slate-200 bg-white py-8 sm:py-11">
            ...
            <div className="mt-4 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
              {SOFTWARE_DIRECTORY_POPULAR_CATEGORIES.map((category) => (
                <Link
                  key={category.href}
                  href={category.href}
                  className="group flex flex-col rounded-xl border border-slate-200 bg-[#F8FAFC] p-5 shadow-sm transition-all duration-200 hover:border-[#10B981]/40 hover:shadow-md focus:outline-none focus-visible:ring-2 focus-visible:ring-[#10B981] focus-visible:ring-offset-2"
                >
                  <h3 className="text-[#1A2D48] text-lg font-bold group-hover:text-[#10B981]">{category.name}</h3>
                  <p className="mt-1.5 text-[#6E6E6E] text-sm leading-relaxed">{category.description}</p>
        ...
        <section id="browse-by-category" className="border-b border-slate-200 bg-[#F8FAFC] py-8 sm:py-11">
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                        ...
                        {category.secondaryLinks && category.secondaryLinks.length > 0 && (
                          <div className="mt-1.5 flex flex-wrap items-center gap-x-3 gap-y-1 text-sm">
                            {category.secondaryLinks.map((secondary) => (
                              <Link key={secondary.href} href={secondary.href} className="text-[#6E6E6E] hover:text-[#1A2D48] hover:underline">
                                {secondary.label}
                              </Link>
                            ))}
                          </div>
        ...
        <section id="browse-by-trade" className="border-b border-slate-200 bg-white py-8 sm:py-11">
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            ...
            <div className="mt-4 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
              {SOFTWARE_DIRECTORY_TRADE_HUBS.map((trade) => (
                ...
                >
                  ...
                  <p className="mt-1.5 text-[#6E6E6E] text-sm leading-relaxed">{trade.description}</p>
                  <span className="mt-4 inline-block text-sm font-semibold text-[#10B981] group-hover:underline">
                    View trade hub →
                  </span>
                </Link>
        ...
        <section id="how-to-choose-software" className="border-b border-slate-200 bg-[#F8FAFC] py-8 sm:py-11">
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <SectionTitle sub="A quick framework to narrow your options before you start demo calls.">
              How to choose business software
            </SectionTitle>
            <p className="max-w-3xl text-[#6E6E6E] text-sm leading-relaxed sm:text-base">
            ...
            <div className="mt-5 grid gap-5 sm:grid-cols-2">
              {SOFTWARE_DIRECTORY_EDITORIAL_TIPS.map((tip) => (
                <article key={tip.title} className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
                  <h3 className="text-[#1A2D48] text-base font-bold">{tip.title}</h3>
                  <p className="mt-1.5 text-[#6E6E6E] text-sm leading-relaxed">{tip.body}</p>
                </article>
              ))}
            </div>
          </div>
